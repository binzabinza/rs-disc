...
from models import Item, ItemPage, ItemCategory
from utilities import log_manager, progress_bar
import math
import time
...
def get_items_in_category(
        ...
        update_category_block: Callable[[ItemCategory], None]
    ...
    progress_bar(count=0, total=expected_total_count)
    ...
    for letter, count in request_alphas.items():
        ...
        for page_num in range(1, count + 1):
            ...
            page = [
                Item(
                    ...
                    type=i.type,
                    members_only=i.members_only.lower() == 'true'
                )
                for i in page
            ]
            items += page
        ...
        progress_bar(
            ...
            status=f'{total_count}/{expected_total_count} items retrieved.'
        )
    ...
    category.expected_item_count = expected_total_count
